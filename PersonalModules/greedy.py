...
def closest_relay(sink, candidate, sinked_relays, forbidden_sinked_relays):
    ...
    if len(forbidden_sinked_relays) == 0:
        ...
        # Find the closest relay
        for i in range(len(allowed_sinked_relays)):
            if math.dist(candidate, allowed_sinked_relays[i][0]) < 30:
...
def greedy_algorithm(sink, sinkless_sentinels, free_slots, max_hops_number):
    First_time = True
    ...
    while len(sinkless_sentinels) != 0:
        ...
        if math.dist(current_sinkless_sentinel, sink) < 30:
            stop = True
        if len(sinked_relays) != 0:
            a, useless_variable = closest_relay(sink, current_sinkless_sentinel, sinked_relays,
                                                forbidden_sinked_relays)
            if math.dist(current_sinkless_sentinel, a[0]) < 30:  # and max_hops_number >= a[1]
                stop = True
        ...
        while not stop:
            ...
            for j in range(len(free_slots)):
                if math.dist(current_node, free_slots[j]) < 30:
            ...
            if len(candidate_slots) >= 1:
                ...
                free_slots = temp
            elif len(candidate_slots) == 0 and math.dist(current_node, current_sinkless_sentinel) < 30:
